...
__copyright__ = "Copyright (c) 2024 The OpenThread Authors. Modified by Texas Instruments for TI NCP Wi-SUN devices"
__version__ = "0.1.0"
...
# COAP ADC WINDOW consts
COAP_ADC_WINDOW_LOW_ID = 0
COAP_ADC_WINDOW_HIGH_ID = 1
                ...
                if pkt.upper_layer_protocol.type == ipv6.IPV6_NEXT_HEADER_ICMP:
                    ...
                    if udp_pkt.header.dst_port == COAP_PORT:
                        ...
                        # LED GET response
                            ...
                            elif coap_uri_path == 'adc_window':
                                if len(p.payload) == 1:
                                    adc_window = "Middle"
                                    if int(p.payload[0]) == 2:
                                        adc_window = "High"
                                    elif int(p.payload[0]) == 1:  
                                        adc_window = "Low"
                                    print("ADC Window State: ",adc_window)
                                else:
                                    print("Error: ADC Window State")
                            elif coap_uri_path == 'adc_voltage':
                                if len(p.payload) == 2:
                                    adc_voltage = ((int(p.payload[0])) << 8) + int(p.payload[1])
                                    print("ADC voltage: ", adc_voltage)
                                else:
                                    print("Error: ADC voltage")
                            else:
                                print("GET Error: invalid URI path")
                        ...
                        # LED PUT/POST response
                            ...
                            if coap_uri_path == 'led':
                                ...
                                print("ADC Window range successfully set")
                            elif coap_uri_path == 'adc_voltage': 
                                print("ADC voltage measurement started")
                            else: 
                                print("PUT/POST Error: invalid URI path")
        ...
        try:
            ...
            if len(params) > 4:
                for i in range(4, len(params)):
                    ...
                    if params[i] == '--led_state' and len(params) >= (i + 2):
                        if params[i+1] == 'r':
                            resource_target = COAP_RLED_ID
                        elif params[i+1] == 'g':
                            resource_target = COAP_GLED_ID
                        else:
                        ...
                        if params[i+2] == '0':
                            ...
                            return
                    if params[i] == '--adc_window_state' and len(params) >= (i + 2):
                        if params[i+1] == 'low':
                            payload[0] = COAP_ADC_WINDOW_LOW_ID
                        elif params[i+1] == 'high':
                            payload[0] = COAP_ADC_WINDOW_HIGH_ID
                        else:
                            print("Invalid ADC WINDOW target, must be low or high")
                            return

                        if int(params[i+2]) < 0 or int(params[i+2]) > 4095:
                            print("Invalid ADC WINDOW ranges")
                            return

                        payload[1] = int(params[i+2]) >> 8
                        payload[2] = int(params[i+2]) & 0x00FF
            ...
            if params[1] == "get":
                coap_req = self.ipv6_factory.build_coap_request(srcIPAddress, addr, coap_confirm,
                    ipv6.COAP_METHOD_CODE_GET, uri_path, option_list, resource_target, resource_state,
                    payload, tkl=DEFAULT_TKL, token=coap_led_req_token)
            elif params[1] == "put" or params[1] == "post":
                if params[1] == "put":
                    coap_req = self.ipv6_factory.build_coap_request(srcIPAddress, addr, coap_confirm,
                        ipv6.COAP_METHOD_CODE_PUT, uri_path, option_list, resource_target, resource_state,
                        payload, tkl=DEFAULT_TKL, token=coap_led_req_token)
                else:
                    coap_req = self.ipv6_factory.build_coap_request(srcIPAddress, addr, coap_confirm,
                        ipv6.COAP_METHOD_CODE_POST, uri_path, option_list, resource_target, resource_state,
                        payload, tkl=DEFAULT_TKL, token=coap_led_req_token)
            else:
